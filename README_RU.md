...
> Форк под Codex.
> CLI остается `ccbot`, а часть `CLAUDE_*` переменных сохранена для совместимости с upstream-конфигами.

Удалённое управление сессиями Codex через Telegram — мониторинг, интерактивное управление и работа с AI-сессиями в tmux.

https://github.com/user-attachments/assets/15ffb38e-5eb9-4720-93b9-412e4961dc93

## Зачем CCBot?

Codex работает в терминале. Когда вы отходите от компьютера — в дороге, дома или просто не за рабочим местом — сессия продолжает выполняться, но вы теряете видимость и контроль.

CCBot позволяет **бесшовно продолжать ту же самую сессию через Telegram**. Ключевая идея: он работает поверх **tmux**, а не через отдельную SDK/API-сессию. Процесс Codex остаётся в tmux-окне на вашей машине, а CCBot только читает вывод и отправляет нажатия клавиш. Это означает:

- **Переключение с десктопа на телефон в середине работы** — Codex делает рефакторинг? Можно отойти и продолжать наблюдать/отвечать из Telegram.
...
Большинство других Telegram-ботов для Codex используют отдельные agent/API-сессии. Такие сессии изолированы и не продолжаются в вашем терминале. CCBot работает иначе: это тонкий слой управления над tmux, поэтому терминал остаётся источником истины, и вы всегда можете вернуться к нему.

## Возможности

- **Сессии по темам** — каждый Telegram topic 1:1 связан с tmux-окном и Codex-сессией
...
- **Отправка сообщений** — проброс текста в Codex через tmux
- **Проброс slash-команд** — любая `/command` уходит напрямую в Codex (например, `/clear`, `/compact`, `/cost`)
- **Создание новых сессий** — запуск Codex из Telegram через браузер директорий
- **Возобновление сессий** — выберите существующую Codex-сессию в директории, чтобы продолжить с того места, где остановились
...
- **Трекинг сессий через hook** — авто-связывание tmux-окон с Codex-сессиями через `SessionStart`
...
- **Codex** — CLI-инструмент `codex` должен быть установлен
...
| `CLAUDE_COMMAND` | `codex` | Команда запуска в новых окнах (старое имя env-переменной) |
| `CCBOT_CLAUDE_PROJECTS_PATH` | `~/.codex` | Корень transcript-файлов для мониторинга (старое имя env-переменной) |
...
Часть `CLAUDE_*` названий сохранена только ради совместимости.

> Если бот запущен на VPS без интерактивного терминала для подтверждений, можно использовать:
>
> ```
> CLAUDE_COMMAND=IS_SANDBOX=1 codex --dangerously-skip-permissions
> ```

## Отслеживание сессий

По умолчанию этот fork сканирует transcript-файлы в `~/.codex`. Если ваши логи Codex лежат в другом месте, задайте `CCBOT_CLAUDE_PROJECTS_PATH`.

Если вы все еще используете upstream-flow с SessionStart hook:
...
Это записывает отображение window-session в `$CCBOT_DIR/session_map.json` (по умолчанию `~/.ccbot/`), чтобы бот автоматически отслеживал, какая Codex-сессия работает в каждом tmux-окне — даже после `/clear` или рестарта сессии.
...
| `/esc` | Отправить Escape для прерывания Codex |

**Команды Codex (пробрасываются через tmux):**
...
| `/help` | Справка Codex |
| `/memory` | Редактировать AGENTS.md |

Любая неизвестная `/command` также пробрасывается в Codex как есть (например, `/review`, `/doctor`, `/init`).
...
4. Если в каталоге есть существующие Codex-сессии, появится выбор сессий — возобновите существующую или начните новую
5. Будет создано tmux-окно, запустится `codex` (с `--resume` при возобновлении), и ваше отложенное сообщение отправится в сессию

**Отправка сообщений:**

После привязки topic к сессии отправляйте текст или голосовые сообщения в topic — текст уходит в Codex через tmux, голосовые сообщения автоматически транскрибируются и пересылаются как текст.
...
- **Ответах ассистента** — текстовые ответы Codex
...
# Затем запустите Codex в новом окне
codex
```

Окно должно находиться в tmux-сессии `ccbot` (настраивается через `TMUX_SESSION_NAME`). Hook автоматически зарегистрирует его в `session_map.json` при запуске Codex.
...
| `~/.codex/` | Корень transcript-файлов Codex (только чтение) |
...
├── transcript_parser.py   # Парсинг JSONL-транскриптов Codex
...
Спасибо всем, кто вносит вклад. После публикации форка на GitHub здесь можно поставить уже вашу собственную contributor-секцию.
